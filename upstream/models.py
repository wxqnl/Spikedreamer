...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        self.heads["decoder"] = networks.MultiDecoder(
            config, feat_size, shapes, **config.decoder
        )
        if config.reward_head == "symlog_disc":
            self.heads["reward"] = networks.SpikeMLP(
                feat_size,  # pytorch version
                (255,),
                config.reward_layers,
                config.units,
                config.act,
                eval("config."+config.act),
                config.norm,
                eval("config."+config.norm),
        ...
        self.heads["cont"] = networks.SpikeMLP(
            feat_size,  # pytorch version
            [],
            config.cont_layers,
            config.units,
...
class ImagBehavior(nn.Module):
    ...
    def _imagine(self, start, policy, horizon, repeats=None):
        ...
        for k, v in start.items():
            if k == "deter":
                # deter [spike_times, batch, timesteps, dims]
                fla_start[k] = v.reshape([int(v.shape[0])]+[-1]+list(v.shape[3:]))
            else:
                fla_start[k] = v.reshape([-1]+list(v.shape[2:]))
        start = fla_start
        def step(prev, _):
            state, _, _ = prev
        ...
        # 将start 和succ组合起来 [start, succ] 
        states = {k: torch.cat([start[k][None], v[:-1]], 0) for k, v in succ.items()}
        if repeats:
            raise NotImplemented("repeats is not implemented in this version")
        feats = tools.swap(feats, (0, 1), (1, 0))
        states["deter"] = tools.swap(states["deter"], (0, 1), (1, 0))
