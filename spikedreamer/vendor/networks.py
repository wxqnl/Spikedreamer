# Derived from Brain-Cog-Lab/Spiking-WM, Apache-2.0.
# Modified for SpikeDreamer on 2026-09-08; see docs/upstream.md.
import math
import numpy as np
import re
...
from . import node, normalization
...
class MultiEncoder(nn.Module):
    ...
    def forward(self, obs):
        ...
        if self.mlp_shapes:
            ...
            raise ValueError("Use the visual encoder configuration; proprioception is not supported.")
...
class SpikeMLP(nn.Module):
    ...
    def dist(self, dist, mean, std, shape):
        # Keep probabilistic readouts stable under autocast; convolutions and
        # projections still run in the selected mixed precision.
        mean = mean.float()
        std = std.float() if torch.is_tensor(std) else std
        if dist == "normal":
            return tools.ContDist(
