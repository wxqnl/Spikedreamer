...
class OneHotDist(torchd.one_hot_categorical.OneHotCategorical):
    def __init__(self, logits=None, probs=None, unimix_ratio=0.0):
        ...
        probs = probs.float() if probs is not None else None
...
def swap(x, i1=(0,1), i2=(1, 0)):
    assert len(i1) ==  len(i2)
    ori_shape = list(range(len(x.shape)))
    tar_shape = list(range(len(x.shape)))
    for k in range(len(i1)):
        tar_shape[i1[k]] = ori_shape[i2[k]]
    return x.permute(tar_shape)
