...
# Adapted from NM512/dreamerv3-torch (MIT), retrieved 2026-09-12.
# Original snapshot and license: ../../upstream/ann-dreamer/.
from . import tools
...
class ConvEncoder(nn.Module):
    ...
    def forward(self, obs):
        # The shared trainer has already centered image inputs at zero.
...
class ConvDecoder(nn.Module):
    ...
    def forward(self, features, dtype=None):
        ...
        if self._cnn_sigmoid:
            mean = F.sigmoid(mean)
        # The shared trainer reconstructs zero-centered image targets.
        return mean
...
class MLP(nn.Module):
    ...
    def dist(self, dist, mean, std, shape):
        if dist == "tanh_normal":
            ...
            dist = tools.ContDist(
                torchd.independent.Independent(dist, 1)
            )
            ...
            dist = tools.ContDist(
                torchd.gumbel.Gumbel(mean, 1 / self._temp)
            )
        elif dist == "huber":
            dist = tools.ContDist(
